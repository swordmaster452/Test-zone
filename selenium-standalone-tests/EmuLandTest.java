...
public class EmuLandTest {
    ...
    public void setUp() throws MalformedURLException {
        // здесь настройка параметров браузера
        ChromeOptions options = new ChromeOptions();
        options.setPageLoadStrategy(org.openqa.selenium.PageLoadStrategy.EAGER);
        // Подключение к  Selenium Standalone серверу
    ...
    public void testSearchFunction() throws InterruptedException {
        ...
        // Находим поле поиска по name="q" нашел через devtools
        WebElement searchInput = driver.findElement(By.name("q"));
        searchInput.clear();

        // определенная строка для ввода и проверки
    ...
    public void testNavigationToSega() throws InterruptedException {
        ...
        // Кликаем по кнопке главного меню "АРКАДНЫЕ"
        String targetSection = "АРКАДНЫЕ";
        WebElement menuLink = driver.findElement(By.linkText(targetSection));
        menuLink.click();

        // Небольшая пауза для смены страницы
        Thread.sleep(1500);


        String currentUrl = driver.getCurrentUrl().toLowerCase();

        // Проверяем, что в URL появилось ключевое слово раздела Аркада
    ...
    public void testNegativeLogin() {
        ...
        // Находим поле Логин по атрибуту placeholder и вводим свои неверные данные
        ...
        //  ожидание на максимум 10 секунд
        org.openqa.selenium.support.ui.WebDriverWait wait =
                new org.openqa.selenium.support.ui.WebDriverWait(driver, java.time.Duration.ofSeconds(10));

        // Ждем появление надписи "Такого пользователя не существует"
    ...
    @Test
    @DisplayName("Тест 4: Переход в раздел Ссылки")
    public void testNavigationToLinks() throws InterruptedException {
        // Открывает страницу сайта
        driver.get("https://emu-land.net");

        // Находим ссылку по тегу и атрибуту href
        WebElement linksMenu = driver.findElement(By.cssSelector("a[href='/links']"));

        //
        org.openqa.selenium.JavascriptExecutor js = (org.openqa.selenium.JavascriptExecutor) driver;
        js.executeScript("arguments[0].click();", linksMenu);

        // Небольшая пауза для прогрузки
        Thread.sleep(1500);

        // Проверяем, что в текущем URL-адресе появилось слово 'links'
        String currentUrl = driver.getCurrentUrl().toLowerCase();
        boolean isSuccess = currentUrl.contains("links");

        // Вывод сообщения об успехе в консоль
        if (isSuccess) {
            System.out.println("переход в раздел ссылки завершен успешно");
        }

        //
        Assertions.assertTrue(isSuccess,
                "Не удалось перейти в раздел 'Ссылки'! Текущий URL: " + driver.getCurrentUrl());
    }
